...
bool ultraCooldown = false;
int ultraCooldownCNT = 0;
...
void setup() {
  ...
  Serial.println("Ultrasonic Threshold: " + String(ultraThres));
}
...
void loop() {
 ...
 showNewData();
 CheckTimer();
 if(ReadUltrasonic()<=ultraThres){
  if(!ultraCooldown&&ultraCooldownCNT>70)Serial.println("<USONIC>");
  ultraCooldown = true;
  ultraCooldownCNT = 0;
 }else{
  ultraCooldown = false;
...
/*if(reading == first_reading){
 ...
 //Serial.println(ultraCooldownCNT);
 if(ultraCooldownCNT>60000)ultraCooldownCNT=300;
 ultraCooldownCNT++;
}
